...
function plannings_edit(params) {
  ...
  function update_planning(event, ui) {
    var data = $('#planning > ol > li').map(function () {
      return {
        ...
        'destinations': $('ul.sortable li[data-destination_id]', this).map(function () {
          return {
  ...
  function display_planning(data) {
    ...
    if (!progress_dialog(data.matrix, $("#dialog-matrix"), display_planning, '/plannings/' + planning_id + '.json', '/api/0.1/customers/job_matrix.json')) {
      return;
    }
    if (!progress_dialog(data.optimizer, $("#dialog-optimizer"), display_planning, '/plannings/' + planning_id + '.json', '/api/0.1/customers/job_optimizer.json')) {
      return;
    }
    ...
    $.each(data.stores, function(i, store) {
      L.marker(new L.LatLng(store.lat, store.lng), {
        icon: L.icon({
          iconUrl: store.icon,
          iconSize: [32, 32],
          iconAnchor: [16, 16]
    ...
    $.each(data.routes, function (i, route) {
      ...
      $.each(route.stops, function (index, stop) {
        ...
        if (stop.destination.lat && stop.destination.lng) {
          ...
          }).addTo(layers[route.route_id]).addTo(layers_cluster[route.route_id]).bindPopup(SMT['stops/show'](stop));
          m.number = stop.number;
          m.icon_large = route.icon_large;
    ...
    $(".route .export_tomtom a").click(function() {
      var url = this.href;
      $.ajax({
        type: "get",
        url: url,
    ...
    $(".routes")
      ...
      .on("click", ".marker", function (event, ui) {
        ...
        if (destination_id in markers) {
          map.setView(markers[destination_id].getLatLng(), 17, {
            reset: true
          });
          var route_id = $(this).closest("[data-route_id]").attr("data-route_id");
          layers_cluster[route_id].zoomToShowLayer(markers[destination_id], function () {
            markers[destination_id].openPopup();
          });
        }
        return false;
      })
    ...
    $(".automatic_insert").click(function (event, ui) {
      var destination_id = $(this).closest("[data-destination_id]").attr("data-destination_id");
      $.ajax({
        type: "patch",
        url: '/plannings/' + planning_id + '/automatic_insert/' + destination_id + '.json',
        beforeSend: beforeSendWaiting,
        success: display_planning,
        complete: completeWaiting,
        error: ajaxError
  ...
  $("form").submit(function (event) {
    if (initial_zoning != $("#planning_zoning_id").val()) {
