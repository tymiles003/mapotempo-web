...
class Planning < ApplicationRecord
  ...
  def set_routes(routes_visits, recompute = true, ignore_errors = false)
    ...
    if routes_visits.size <= routes.size - 1
      ...
      routes.find{ |r| !r.vehicle_usage_id }.set_visits((customer.visits - visits).select{ |visit|
        tags_compatible?(visit.tags.to_a | visit.destination.tags.to_a)
      })

      index_routes = (1..routes.size).to_a
      routes_visits.each{ |_ref, r|
        index_routes.delete(routes.index{ |rr| rr.vehicle_usage_id && rr.vehicle_usage.vehicle.ref == r[:ref_vehicle] }) if r[:ref_vehicle]
      }
      routes_visits.each{ |ref, r|
        i = routes.index{ |rr| r[:ref_vehicle] && rr.vehicle_usage_id && rr.vehicle_usage.vehicle.ref == r[:ref_vehicle] } || index_routes.shift
        routes[i].ref = ref
  ...
  def default_routes
    if vehicle_usage_set && routes.length != vehicle_usage_set.vehicle_usages.select(&:active).length + 1
      ...
      if !split_by_zones(visits_compatibles)
        routes.find{ |r| !r.vehicle_usage_id }.default_stops
      end
    end
  end
