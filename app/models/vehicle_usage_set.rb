  ...
  def update_out_of_date
    ...
    if open_changed? || close_changed? || store_start_id_changed? || store_stop_id_changed? || rest_start_changed? || rest_stop_changed? || rest_duration_changed? || store_rest_id_changed?
      vehicle_usages.each{ |vehicle_usage|
        if (open_changed? && vehicle_usage.open == open) || (close_changed? && vehicle_usage.close == close) || (store_start_id_changed? && vehicle_usage.store_start_id == store_start_id) ||
          (store_stop_id_changed? && vehicle_usage.store_stop_id == store_stop_id) || (rest_start_changed? && vehicle_usage.rest_start == rest_start) || (rest_stop_changed? && vehicle_usage.rest_stop == rest_stop) ||
          (rest_duration_changed? && vehicle_usage.rest_duration == rest_duration) || (store_rest_id_changed? && vehicle_usage.store_rest_id == store_rest)
