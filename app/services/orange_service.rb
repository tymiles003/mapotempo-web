...
class OrangeService
  ...
  URL = "https://m2m-services.ft-dm.com"

  attr_reader :route, :customer, :auth, :cache_object

  def initialize params
    @cache_object = ActiveSupport::Cache::FileStore.new(File.join(Dir.tmpdir, 'devices'), namespace: 'orange', expires_in: 30)
    @route = params[:route]
  ...
  def test_list
    send_request list_operations(auth[:user], auth[:password], { eqpid: "", dtdeb: Time.now.beginning_of_day, dtfin: Time.now.end_of_day })
  end
  ...
  # def net_request options
  #   RestClient::Request.execute method: :get, user: options[:user], password: options[:password], url: URL + options[:path], headers: { params: options[:params] }
  # end

  def get_positions
    net_request user: customer.orange_user, password: customer.orange_password, path: "/webservices/getpositions.php", params: { ext: "xml" }
  end

  def get_vehicles
    net_request user: customer.orange_user, password: customer.orange_password, path: "/webservices/getvehicles.php", params: { ext: "xml" }
  end

  def list_operations user, password, options
    net_request user: user, password: password, path: "/pnd/index.php", params: { ext: "xml", ref: "", vehid: "", typ: "mis" }
  end

  def send_xml_file options={}
    f = Tempfile.new Time.now.to_i.to_s ; f.write to_xml(options) ; f.rewind
    response = RestClient::Request.execute method: :post, user: customer.orange_user, password: customer.orange_password, url: URL + "/pnd/index.php", payload: { multipart: true, file: f }
    f.unlink
    return response
  end

  def p_time time
    route.planning.date.to_time + (time - Time.new(2000, 1, 1, 0, 0, 0, '+00:00'))
  end
