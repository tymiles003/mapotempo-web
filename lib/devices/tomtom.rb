      ...
      route.stops.select(&:active).each{ |stop|
        ...
        if (position && !position.lat.nil? && !position.lng.nil?) || position.is_a?(StopRest)
          description = [
            ...
            stop.is_a?(StopVisit) ? (stop.visit.take_over ? '(' + stop.visit.take_over_time_in_seconds + ')' : nil) : route.vehicle_usage.default_rest_duration_time_in_seconds,
            stop.open1 || stop.close1 ? (stop.open1 ? stop.open1_time : '') + '-' + (stop.close1_time || '') : nil,
            stop.open2 || stop.close2 ? (stop.open2 ? stop.open2_time : '') + '-' + (stop.close2_time || '') : nil,
            stop.detail,
            stop.comment,
            stop.phone_number,
          ].compact.join(' ').strip
