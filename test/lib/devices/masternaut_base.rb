      ...
      names.each do |name|
        case name
          when :poi_wsdl
            ...
            url = "%s://%s:%s@%s%s" % [ api_url.scheme, @customer.masternaut_user, @customer.masternaut_password, api_url.host, api_url.path + "/POI?wsdl" ]
          ...
          when :poi
            ...
            api_url = URI.parse Mapotempo::Application.config.devices.masternaut.api_url
            url = "%s://%s:%s@%s%s" % [ api_url.scheme, @customer.masternaut_user, @customer.masternaut_password, api_url.host, api_url.path + "/POI" ]
          ...
          when :job_wsdl
            ...
            api_url = URI.parse Mapotempo::Application.config.devices.masternaut.api_url
            url = "%s://%s:%s@%s%s" % [ api_url.scheme, @customer.masternaut_user, @customer.masternaut_password, api_url.host, api_url.path + "/Job?wsdl" ]
            stubs << stub_request(:get, url).to_return(status: 200, body: expected_response)
          when :job
            expected_response = File.read(Rails.root.join("test/web_mocks/masternaut/blank.xml")).strip
            api_url = URI.parse Mapotempo::Application.config.devices.masternaut.api_url
            url = "%s://%s:%s@%s%s" % [ api_url.scheme, @customer.masternaut_user, @customer.masternaut_password, api_url.host, api_url.path + "/Job" ]
            stubs << stub_request(:post, url).to_return(status: 200, body: expected_response)
        end
      end
      yield
